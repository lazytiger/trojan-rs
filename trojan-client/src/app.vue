  ...
  <v-app>
    <v-main class="bg-grey-lighten-4">
      <v-container class="mx-auto" style="max-width: 480px;">
        <v-text-field v-model="config.iface_name" label="虚拟网卡名" variant="outlined"></v-text-field>
        <v-text-field v-model="config.server_domain" label="服务器域名" variant="outlined"></v-text-field>
        <v-text-field v-model="config.server_auth" :append-icon="show ? 'mdi-eye' : 'mdi-eye-off'"
                      :type="show ? 'text' : 'password'" label="服务器密码" variant="outlined"
                      @click:append="show = !show"></v-text-field>
        <v-text-field v-model="config.default_dns" :rules="[check_ipv4]" label="默认DNS"
        ...
        <v-combobox v-model="config.log_level"
                    ...
                    variant="solo"
        ></v-combobox>
        <v-slider v-model="config.pool_size" label="连接池大小" max="20" min="0" step="1">
        ...
        <v-row>
          <v-checkbox v-model="config.enable_ipset" label="全局代理"></v-checkbox>
          <v-checkbox v-model="config.inverse_route" label="反转地址"></v-checkbox>
        </v-row>
        <v-container class="rounded-xl, border">
          <v-checkbox v-model="config.enable_dns" label="信任DNS"
                      @click="config.enable_dns=!config.enable_dns"></v-checkbox>
          <div v-if="config.enable_dns">
            <v-text-field v-model="config.dns_listen" :rules="[check_ipv4]" label="监听地址"
                          variant="outlined"></v-text-field>
            <v-text-field v-model="config.trust_dns" :rules="[check_ipv4]" label="可信DNS地址"
                          variant="outlined"></v-text-field>
          </div>
        </v-container>
